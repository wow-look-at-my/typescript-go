...
func TestUntitledPathHandling(t *testing.T) {
	...
	require.Equal(t, rootLength, 2, "GetEncodedRootLength should return 2 for untitled paths")

	// IsRootedDiskPath should return true
	isRooted := tspath.IsRootedDiskPath(untitledPath)
	require.True(t, isRooted, "IsRootedDiskPath should return true for untitled paths")
	...
	require.Equal(t, string(path), "^/untitled/ts-nul-authority/Untitled-2", "ToPath should not resolve untitled paths against current directory")
	...
	require.Equal(t, normalized, "^/untitled/ts-nul-authority/Untitled-2", "GetNormalizedAbsolutePath should not resolve untitled paths")
}
...
func TestUntitledPathEdgeCases(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.path, func(t *testing.T) {
			...
			require.Equal(t, rootLength, tc.expected, "GetEncodedRootLength for path %s", tc.path)

			isRooted := tspath.IsRootedDiskPath(tc.path)
			require.Equal(t, isRooted, tc.isRooted, "IsRootedDiskPath for path %s", tc.path)
		})
	}
}
