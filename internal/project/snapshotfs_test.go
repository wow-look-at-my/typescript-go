...
func TestSnapshotFSBuilder(t *testing.T) {
	...
	t.Run("builds directory tree on file add", func(t *testing.T) {
		...
		require.True(t, fh != nil, "file should exist")
		require.Equal(t, fh.Content(), "const foo = 1;")
		...
		require.True(t, ok, "/src directory should exist")
		_, hasFoo := srcDir[tspath.Path("/src/foo.ts")]
		require.True(t, hasFoo, "/src should contain /src/foo.ts")

		// / should contain /src
		rootDir, ok := snapshot.diskDirectories[tspath.Path("/")]
		require.True(t, ok, "/ directory should exist")
		_, hasSrc := rootDir[tspath.Path("/src")]
		require.True(t, hasSrc, "/ should contain /src")
	})
	...
	t.Run("removes directory entries on file delete", func(t *testing.T) {
		...
		require.True(t, !hasFile, "file should be deleted")

		// Directory tree should be cleaned up
		_, hasSrcDir := snapshot.diskDirectories[tspath.Path("/src")]
		require.True(t, !hasSrcDir, "/src directory should be removed")

		_, hasRootDir := snapshot.diskDirectories[tspath.Path("/")]
		require.True(t, !hasRootDir, "root directory should be removed")
	})
	...
	t.Run("removes only empty directories on file delete", func(t *testing.T) {
		...
		_, hasFile := snapshot.diskFiles[tspath.Path("/src/foo.ts")]
		require.True(t, !hasFile, "foo.ts should be deleted")

		// bar.ts should still exist
		_, hasBar := snapshot.diskFiles[tspath.Path("/src/bar.ts")]
		require.True(t, hasBar, "bar.ts should still exist")

		// /src directory should still exist with bar.ts
		srcDir, hasSrcDir := snapshot.diskDirectories[tspath.Path("/src")]
		require.True(t, hasSrcDir, "/src directory should still exist")
		_, hasFoo := srcDir[tspath.Path("/src/foo.ts")]
		require.True(t, !hasFoo, "/src should not contain foo.ts")
		_, hasBarInDir := srcDir[tspath.Path("/src/bar.ts")]
		require.True(t, hasBarInDir, "/src should contain bar.ts")

		// root should still contain /src
		rootDir, hasRootDir := snapshot.diskDirectories[tspath.Path("/")]
		require.True(t, hasRootDir, "root directory should still exist")
		_, hasSrc := rootDir[tspath.Path("/src")]
		require.True(t, hasSrc, "root should contain /src")
	})
	...
	t.Run("adds file to existing directory", func(t *testing.T) {
		...
		require.True(t, fh != nil, "bar.ts should exist")
		...
		_, hasFoo := srcDir[tspath.Path("/src/foo.ts")]
		require.True(t, hasFoo, "/src should contain foo.ts")
		_, hasBar := srcDir[tspath.Path("/src/bar.ts")]
		require.True(t, hasBar, "/src should contain bar.ts")
	})
	...
	t.Run("no change when no files added or deleted", func(t *testing.T) {
		...
		snapshot, changed := builder.Finalize()
		require.True(t, !changed, "should not have changed")
	...
	t.Run("overlay files are returned over disk files", func(t *testing.T) {
		...
		fh := builder.GetFile("/src/foo.ts")
		require.True(t, fh != nil)
		require.Equal(t, fh.Content(), "const foo = 999;")
	})
	...
	t.Run("multiple files added and deleted in single cycle", func(t *testing.T) {
		...
		require.True(t, hasSrcInRoot, "root should contain /src")
		_, hasLibInRoot := rootDir[tspath.Path("/lib")]
		require.True(t, hasLibInRoot, "root should contain /lib")
		_, hasOtherInRoot := rootDir[tspath.Path("/other")]
		require.True(t, !hasOtherInRoot, "root should not contain /other")
	})
	...
	t.Run("overlay directories are computed from overlays", func(t *testing.T) {
		...
		require.True(t, ok, "/src overlay directory should exist")
		_, hasOverlay := srcDir[tspath.Path("/src/overlay.ts")]
		require.True(t, hasOverlay, "/src should contain overlay.ts")
		_, hasNested := srcDir[tspath.Path("/src/nested")]
		require.True(t, hasNested, "/src should contain nested/")

		nestedDir, ok := builder.overlayDirectories[tspath.Path("/src/nested")]
		require.True(t, ok, "/src/nested overlay directory should exist")
		_, hasDeep := nestedDir[tspath.Path("/src/nested/deep.ts")]
		require.True(t, hasDeep, "/src/nested should contain deep.ts")

		rootDir, ok := builder.overlayDirectories[tspath.Path("/")]
		require.True(t, ok, "/ overlay directory should exist")
		_, hasSrc := rootDir[tspath.Path("/src")]
		require.True(t, hasSrc, "/ should contain /src")
	})
	...
	t.Run("GetAccessibleEntries combines disk and overlay", func(t *testing.T) {
		...
		require.True(t, slices.Contains(entries.Files, "disk.ts"), "should contain disk.ts")
		require.True(t, slices.Contains(entries.Files, "overlay.ts"), "should contain overlay.ts")
	})
}
...
func TestSnapshotFS(t *testing.T) {
	...
	t.Run("GetFile returns overlay file", func(t *testing.T) {
		...
		require.True(t, fh != nil)
		require.Equal(t, fh.Content(), "overlay content")
	})
	...
	t.Run("GetFile returns disk file when not in overlay", func(t *testing.T) {
		...
		fh := snapshot.GetFile("/src/foo.ts")
		require.True(t, fh != nil)
		require.Equal(t, fh.Content(), "disk content")
	})
	...
	t.Run("GetFile reads from fs when not cached", func(t *testing.T) {
		...
		fh := snapshot.GetFile("/src/foo.ts")
		require.True(t, fh != nil)
		require.Equal(t, fh.Content(), "fs content")
	})
	...
	t.Run("GetFile returns nil for non-existent file", func(t *testing.T) {
		...
		require.True(t, fh == nil, "should return nil for non-existent file")
	})
	...
	t.Run("isOpenFile returns true for overlays", func(t *testing.T) {
		...
		require.True(t, snapshot.isOpenFile("/src/foo.ts"), "overlay file should be open")
		require.True(t, !snapshot.isOpenFile("/src/bar.ts"), "non-overlay file should not be open")
	})
...
func TestSourceFS(t *testing.T) {
	...
	t.Run("tracks files when tracking enabled", func(t *testing.T) {
		...
		require.True(t, !sourceFS.Seen(tspath.Path("/src/foo.ts")))
	...
	t.Run("DisableTracking stops tracking", func(t *testing.T) {
		...
		require.True(t, !sourceFS.Seen(tspath.Path("/src/bar.ts")))
	})
	...
	t.Run("FileExists returns true for files in source", func(t *testing.T) {
		...
		require.True(t, sourceFS.FileExists("/src/foo.ts"))
		require.True(t, !sourceFS.FileExists("/src/nonexistent.ts"))
	})
...
func TestAutoImportBuilderFS(t *testing.T) {
	...
	t.Run("symlink cache mismatch: file cached at symlink path, missed at realpath after deletion", func(t *testing.T) {
		...
		require.Equal(t, realpathPath, "/real/pkg/index.d.ts", "Realpath should resolve the symlink to the real path")
		...
		require.True(t, fh != nil, "File should be readable via symlink path")
		require.Equal(t, fh.Content(), "export declare const x: number;")
		...
		require.True(t, fh2 == nil, "File should be nil when accessed by realpath after deletion from disk")
	})
}
