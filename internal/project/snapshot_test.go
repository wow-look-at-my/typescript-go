...
func TestSnapshot(t *testing.T) {
	...
	t.Run("compilerHost gets frozen with snapshot's FS only once", func(t *testing.T) {
		...
		require.Equal(t, snapshotAfter.ProjectCollection.ConfiguredProject(tspath.Path("/home/projects/ts/p1/tsconfig.json")).ProgramUpdateKind, ProgramUpdateKindCloned)
		// Inferred project wasn't updated last snapshot change, so its program update kind is still NewFiles
		require.Equal(t, snapshotBefore.ProjectCollection.InferredProject(), snapshotAfter.ProjectCollection.InferredProject())
		require.Equal(t, snapshotAfter.ProjectCollection.InferredProject().ProgramUpdateKind, ProgramUpdateKindNewFiles)
		// host for inferred project should not change
		require.Equal(t, snapshotAfter.ProjectCollection.InferredProject().host.sourceFS.source, snapshotBefore.fs)
	})
	...
	t.Run("cached disk files are cleaned up", func(t *testing.T) {
		...
		assert.True(t, snapshotBefore.fs.diskFiles["/home/projects/ts/p1/a.ts"] != nil)
		assert.True(t, snapshotBefore.fs.diskFiles["/home/projects/ts/p2/b.ts"] != nil)
		...
		assert.True(t, snapshotAfter.fs.diskFiles["/home/projects/ts/p1/a.ts"] == nil)
		assert.True(t, snapshotAfter.fs.diskFiles["/home/projects/ts/p2/b.ts"] != nil)
	})
	...
	t.Run("GetFile returns nil for non-existent files", func(t *testing.T) {
		...
		assert.True(t, handle == nil, "GetFile should return nil for non-existent file")

		// Test that ReadFile returns false for non-existent file
		_, ok := snapshot.ReadFile("/home/projects/TS/p1/nonexistent.ts")
		assert.True(t, !ok, "ReadFile should return false for non-existent file")
	})
