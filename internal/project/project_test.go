...
func TestProjectProgramUpdateKind(t *testing.T) {
	...
	t.Run("NewFiles on initial build", func(t *testing.T) {
		...
		require.True(t, configured != nil)
		require.Equal(t, configured.ProgramUpdateKind, project.ProgramUpdateKindNewFiles)
	})
	...
	t.Run("Cloned on single-file change", func(t *testing.T) {
		...
		configured := snapshot.ProjectCollection.ConfiguredProject(tspath.Path("/src/tsconfig.json"))
		require.True(t, configured != nil)
		require.Equal(t, configured.ProgramUpdateKind, project.ProgramUpdateKindCloned)
	})
	...
	t.Run("SameFileNames on config change without root changes", func(t *testing.T) {
		...
		configured := snapshot.ProjectCollection.ConfiguredProject(tspath.Path("/src/tsconfig.json"))
		require.True(t, configured != nil)
		require.Equal(t, configured.ProgramUpdateKind, project.ProgramUpdateKindSameFileNames)
	})
...
func TestProject(t *testing.T) {
	...
	t.Run("commandLineWithTypingsFiles is reset on CommandLine change", func(t *testing.T) {
		...
		require.True(t, len(npmCalls) > 0, "expected at least one npm install call from ATA")
...
func TestPushDiagnostics(t *testing.T) {
	...
	t.Run("publishes program diagnostics on initial program creation", func(t *testing.T) {
		...
		require.True(t, len(calls) > 0, "expected at least one PublishDiagnostics call")
		...
		require.True(t, tsconfigCall != nil, "expected PublishDiagnostics call for tsconfig.json")
		require.True(t, len(tsconfigCall.Params.Diagnostics) > 0, "expected at least one diagnostic")
	})
	...
	t.Run("clears diagnostics when project is removed", func(t *testing.T) {
		...
		require.True(t, len(firstProjectCalls) >= 2, "expected at least 2 PublishDiagnostics calls for first project")
		// Last call should clear diagnostics
		lastCall := firstProjectCalls[len(firstProjectCalls)-1]
		require.Equal(t, len(lastCall.Params.Diagnostics), 0, "expected empty diagnostics after project cleanup")
	})
	...
	t.Run("updates diagnostics when program changes", func(t *testing.T) {
		...
		calls := utils.Client().PublishDiagnosticsCalls()
		require.True(t, len(calls) > initialCallCount, "expected additional PublishDiagnostics call after change")
		...
		require.True(t, lastTsconfigCall != nil, "expected PublishDiagnostics call for tsconfig.json")
		// After fixing the error, there should be no program diagnostics
		require.Equal(t, len(lastTsconfigCall.Params.Diagnostics), 0, "expected no diagnostics after removing baseUrl option")
	})
	...
	t.Run("does not publish for inferred projects", func(t *testing.T) {
		...
		require.Equal(t, len(calls), 0, "expected no PublishDiagnostics calls for inferred projects")
	})
}
