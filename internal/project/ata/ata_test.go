...
func TestATA(t *testing.T) {
	...
	t.Run("local module should not be picked up", func(t *testing.T) {
		...
		require.True(t, configFile != nil, "local config.js should be included")
		...
		require.Equal(t, len(npmCalls), 1)
		require.Equal(t, npmCalls[0].Args[2], "types-registry@latest")
	})
	...
	t.Run("configured projects", func(t *testing.T) {
		...
		session.WaitForBackgroundTasks()
		npmCalls := utils.NpmExecutor().NpmInstallCalls()
		require.Equal(t, len(npmCalls), 2)
		require.Equal(t, npmCalls[0].Cwd, projecttestutil.TestTypingsLocation)
		require.Equal(t, npmCalls[0].Args[2], "types-registry@latest")
		require.Equal(t, npmCalls[1].Cwd, projecttestutil.TestTypingsLocation)
		require.True(t, slices.Contains(npmCalls[1].Args, "@types/jquery@latest"))
		require.Equal(t, len(utils.Client().RefreshDiagnosticsCalls()), 1)
	})
	...
	t.Run("inferred projects", func(t *testing.T) {
		...
		require.Equal(t, 2, len(calls), "Expected exactly 2 npm install calls")
		require.Equal(t, calls[0].Cwd, projecttestutil.TestTypingsLocation)
		require.Equal(t, calls[0].Args, []string{"install", "--ignore-scripts", "types-registry@latest"})
		require.Equal(t, calls[1].Cwd, projecttestutil.TestTypingsLocation)
		require.Equal(t, calls[1].Args[2], "@types/jquery@latest")
		...
		require.True(t, jqueryTypesFile != nil, "jquery types should be installed")
	})
	...
	t.Run("type acquisition with disableFilenameBasedTypeAcquisition:true", func(t *testing.T) {
		...
		calls := utils.NpmExecutor().NpmInstallCalls()
		require.Equal(t, 1, len(calls), "Expected exactly 1 npm install call")
		require.Equal(t, calls[0].Cwd, projecttestutil.TestTypingsLocation)
		require.Equal(t, calls[0].Args, []string{"install", "--ignore-scripts", "types-registry@latest"})
	})
	...
	t.Run("discover from node_modules empty types has import", func(t *testing.T) {
		...
		require.Equal(t, calls[0].Args, []string{"install", "--ignore-scripts", "types-registry@latest"})
		require.True(t, slices.Contains(calls[1].Args, "@types/jquery@latest"))
	})
	...
	t.Run("Malformed package.json should be watched", func(t *testing.T) {
		...
		require.NoError(t, utils.FS().WriteFile(
			"/user/username/projects/project/package.json",
		...
		calls = utils.NpmExecutor().NpmInstallCalls()
		require.Equal(t, 2, len(calls))
		require.True(t, slices.Contains(calls[1].Args, "@types/commander@latest"))

		// Verify types file present
		ls, err := session.GetLanguageService(context.Background(), uri)
		require.NoError(t, err)
		program := ls.GetProgram()
		require.True(t, program.GetSourceFile(projecttestutil.TestTypingsLocation+"/node_modules/@types/commander/index.d.ts") != nil)
	})
	...
	t.Run("should redo resolution that resolved to '.js' file after typings are installed", func(t *testing.T) {
		...
		require.True(t, program.GetSourceFile("/user/username/projects/node_modules/commander/index.js") == nil)
	})
	...
	t.Run("expired cache entry (inferred project, should install typings)", func(t *testing.T) {
		...
		require.Equal(t, program.GetSourceFile(projecttestutil.TestTypingsLocation+"/node_modules/@types/jquery/index.d.ts").Text(), "export const y = 10")
	})
	...
	t.Run("non-expired cache entry (inferred project, should not install typings)", func(t *testing.T) {
		...
		require.Equal(t, program.GetSourceFile(projecttestutil.TestTypingsLocation+"/node_modules/@types/jquery/index.d.ts").Text(), "export const x = 10;")
	})
	...
	t.Run("deduplicate from local @types packages", func(t *testing.T) {
		...
		require.Equal(t, len(npmCalls), 1)
		require.Equal(t, npmCalls[0].Cwd, projecttestutil.TestTypingsLocation)
		require.Equal(t, npmCalls[0].Args, []string{"install", "--ignore-scripts", "types-registry@latest"})

		// And the program should include the local @types/node declaration file
		ls, err := session.GetLanguageService(context.Background(), uri)
		require.NoError(t, err)
		program := ls.GetProgram()
		require.True(t, program.GetSourceFile("/user/username/projects/project/node_modules/@types/node/index.d.ts") != nil)
	})
	...
	t.Run("should install typings for unresolved imports", func(t *testing.T) {
		...
		require.Equal(t, calls[1].Cwd, projecttestutil.TestTypingsLocation)
		require.Equal(t, calls[1].Args[0], "install")
		require.Equal(t, calls[1].Args[1], "--ignore-scripts")
		// Check that all three packages are in the install command
		installArgs := calls[1].Args
		require.True(t, slices.Contains(installArgs, "@types/ember__component@latest"))
		require.True(t, slices.Contains(installArgs, "@types/commander@latest"))
		require.True(t, slices.Contains(installArgs, "@types/node@latest"))
		...
		require.True(t, nodeTypesFile != nil, "node types should be installed")
		commanderTypesFile := program.GetSourceFile(projecttestutil.TestTypingsLocation + "/node_modules/@types/commander/index.d.ts")
		require.True(t, commanderTypesFile != nil, "commander types should be installed")
		...
		require.True(t, emberComponentTypesFile != nil, "ember__component types should be installed")
	})
