...
func TestBulkCacheInvalidation(t *testing.T) {
	...
	t.Run("large number of node_modules changes invalidates only node_modules cache", func(t *testing.T) {
		...
		test := func(t *testing.T, fileEvents []*lsproto.FileEvent, expectNodeModulesInvalidation bool) {
			...
			require.Equal(t, ls.GetProgram().Options().Target, core.ScriptTargetES2015)
			...
			require.Equal(t, ls.GetProgram().Options().Target, core.ScriptTargetES2015, "Config should not have been reloaded for node_modules-only changes")

			// Config registry should be the same instance (no configs reloaded)
			require.Equal(t, configBefore, configAfter, "Config registry should not have changed for node_modules-only changes")

			fsDtsText := snapshotAfter.GetFile("/project/node_modules/@types/node/fs.d.ts").Content()
			if expectNodeModulesInvalidation {
				require.Equal(t, fsDtsText, "new text")
			} else {
				require.Equal(t, fsDtsText, "")
			}
		}
	...
	t.Run("large number of changes outside node_modules", func(t *testing.T) {
		...
		test := func(t *testing.T, fileEvents []*lsproto.FileEvent, expectConfigReload bool) {
			...
			if expectConfigReload {
				require.Equal(t, ls.GetProgram().Options().Target, core.ScriptTargetESNext, "Config should have been reloaded for changes outside node_modules")
				assert.True(t, ls.GetProgram().GetSourceFile("/project/src/rootFile.ts") != nil, "New root file should be present")
			} else {
				require.Equal(t, ls.GetProgram().Options().Target, core.ScriptTargetES2015, "Config should not have been reloaded for changes outside node_modules")
				assert.True(t, ls.GetProgram().GetSourceFile("/project/src/rootFile.ts") == nil, "New root file should not be present")
			}
		}
	...
	t.Run("large number of changes outside node_modules causes project reevaluation", func(t *testing.T) {
		...
		require.Equal(t, initialProject.Name(), "/project/tsconfig.json", "Should initially use root tsconfig")
		...
		require.NoError(t, err)
		require.Equal(t, ls.GetProgram().Options().Strict, core.TSTrue, "Should initially use strict mode from root config")
		...
		require.Equal(t, newProject.Name(), "/project/src/utils/tsconfig.json", "Should now use nested tsconfig after bulk invalidation")
		require.Equal(t, ls.GetProgram().Options().Strict, core.TSFalse, "Should now use non-strict mode from nested config")
		require.Equal(t, ls.GetProgram().Options().Target, core.ScriptTargetESNext, "Should use esnext target from nested config")
	})
	...
	t.Run("config file names cache", func(t *testing.T) {
		...
		test := func(t *testing.T, fileEvents []*lsproto.FileEvent, expectConfigDiscovery bool, testName string) {
			...
			snapshot, release := session.Snapshot()
			defer release()
			require.True(t, snapshot.ProjectCollection.InferredProject() != nil, "Should have inferred project")
			require.Equal(t, snapshot.GetDefaultProject("file:///project/src/index.ts").Kind, project.KindInferred)
		...
		"include": ["src/**/*"]
			...
			if expectConfigDiscovery {
				...
				require.Equal(t, newProject.Kind, project.KindConfigured, "Should now use configured project after cache invalidation")
				require.Equal(t, newProject.Name(), "/project/tsconfig.json", "Should use the newly discovered tsconfig")
			} else {
				// Should still use inferred project (config file names cache not cleared)
				require.True(t, newProject == snapshot.ProjectCollection.InferredProject(), "Should still use inferred project after node_modules-only changes")
			}
		}
	...
	t.Run("excessive changes in dist folder do not invalidate", func(t *testing.T) {
		...
		defer release()
		newProject := snapshot.GetDefaultProject("file:///project/src/index.ts")
		require.Equal(t, newProject.Kind, project.KindInferred, "dist-folder changes should not cause config discovery")
