...
func TestRefCountingCaches(t *testing.T) {
	...
	t.Run("parseCache", func(t *testing.T) {
		...
		t.Run("reuse unchanged file", func(t *testing.T) {
			...
			require.Equal(t, mainEntry.refCount, 1)
			require.Equal(t, utilsEntry.refCount, 1)
			...
			require.True(t, newMain != main)
			require.True(t, newMainEntry != mainEntry)
			require.Equal(t, ls.GetProgram().GetSourceFile("/user/username/projects/myproject/src/utils.ts"), utils)
			require.Equal(t, mainEntry.refCount, 1)
			require.Equal(t, newMainEntry.refCount, 1)
			require.Equal(t, utilsEntry.refCount, 2)
			release()
			require.Equal(t, mainEntry.refCount, 0)
			require.Equal(t, newMainEntry.refCount, 1)
			require.Equal(t, utilsEntry.refCount, 1)
		})
		...
		t.Run("unchanged program does not over-ref", func(t *testing.T) {
			...
			mainEntry, _ := session.parseCache.entries.Load(NewParseCacheKey(main.ParseOptions(), main.Hash, main.ScriptKind))
			require.Equal(t, mainEntry.refCount, 1, "initial refCount should be 1")
			...
			require.Equal(t, main, main2, "main.ts source file should be reused")

			// main.ts refCount should be 2: one for old program, one for new program
			mainEntry, _ = session.parseCache.entries.Load(NewParseCacheKey(main.ParseOptions(), main.Hash, main.ScriptKind))
			require.Equal(t, mainEntry.refCount, 2, "refCount should be 2 (old and new program)")
			...
			mainEntry, ok := session.parseCache.entries.Load(NewParseCacheKey(main.ParseOptions(), main.Hash, main.ScriptKind))
			require.True(t, ok, "entry should still exist after releasing old snapshot")
			require.Equal(t, mainEntry.refCount, 1, "refCount should be 1 after releasing old snapshot")
			...
			require.True(t, !ok, "entry should be deleted after program is disposed")
		})
	})
	...
	t.Run("extendedConfigCache", func(t *testing.T) {
		...
		t.Run("release extended configs with project close", func(t *testing.T) {
			...
			require.Equal(t, config.ExtendedSourceFiles()[0], "/user/username/projects/myproject/tsconfig.base.json")
			extendedConfigEntry, _ := session.extendedConfigCache.entries.Load("/user/username/projects/myproject/tsconfig.base.json")
			require.Equal(t, extendedConfigEntry.refCount, 1)
			release()

			session.DidCloseFile(context.Background(), "file:///user/username/projects/myproject/src/main.ts")
			session.DidOpenFile(context.Background(), "untitled:Untitled-1", 1, "", lsproto.LanguageKindTypeScript)
			require.Equal(t, extendedConfigEntry.refCount, 0)
		})
	})
}
