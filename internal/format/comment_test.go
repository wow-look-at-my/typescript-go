...
func TestCommentFormatting(t *testing.T) {
    ...
    */
    async x() {}
		...
		assert.True(t, !strings.Contains(firstFormatted, "*/\n   /"), "should not corrupt */ to /")
		assert.True(t, strings.Contains(firstFormatted, "*/"), "should preserve */ token")
		assert.True(t, strings.Contains(firstFormatted, "async"), "should preserve async keyword")
		...
		assert.True(t, !strings.Contains(secondFormatted, " sync x()"), "should not corrupt async to sync")
		assert.True(t, strings.Contains(secondFormatted, "async"), "should preserve async keyword on second pass")
	})
	...
	t.Run("format JSDoc with tab indentation", func(t *testing.T) {
		...
		assert.True(t, !strings.Contains(formatted, " \t*"), "should not have space before tab before asterisk")
		assert.True(t, strings.Contains(formatted, "\t *"), "should have tab before space before asterisk")

		// Verify console.log is properly indented with tabs
		assert.True(t, strings.Contains(formatted, "\t\tconsole.log"), "console.log should be indented with two tabs")
	})
	...
	t.Run("format comment inside multi-line argument list", func(t *testing.T) {
		...
		assert.True(t, strings.Contains(formatted, "\t// the second arg"), "comment should be indented with tab")
	...
	t.Run("format comment in chained method calls", func(t *testing.T) {
		...
		assert.True(t, strings.Contains(formatted, "\t// A second call") || strings.Contains(formatted, "   // A second call"), "comment should be indented")
		// The comment should not lose its indentation
    ...
    const elements = document.querySelectorAll('.test')
		...
		edits := format.FormatDocument(ctx, sourceFile)
		formatted := applyBulkEdits(originalText, edits)
		assert.True(t, len(formatted) > 0, "formatted text should not be empty")
	})
...
`
		...
		// Basic sanity checks
		assert.True(t, len(formatted) > 0, "formatted text should not be empty")
		assert.True(t, strings.Contains(formatted, "_enableDisposeWithListenerWarning"), "should preserve variable name")
	})
}
