...
func TestFormatNoTrailingNewline(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.True(t, !strings.HasSuffix(newText, " "), "Formatter should not add trailing space")
			// Also check that no space was added at EOF position if text didn't end with newline
			if !strings.HasSuffix(tc.text, "\n") {
				require.True(t, !strings.HasSuffix(newText, " "), "Formatter should not add space before EOF")
			}
		})
	}
}
