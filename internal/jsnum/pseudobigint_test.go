...
func TestParsePseudoBigInt(t *testing.T) {
	...
	t.Run("parse non-decimal bases (small numbers)", func(t *testing.T) {
		...
		for _, c := range cases {
			...
			require.Equal(t, got, c.out, "literal: %q", c.lit)
		}
	})

	t.Run("can parse large literals", func(t *testing.T) {
		t.Parallel()
		require.Equal(t, ParsePseudoBigInt("123456789012345678901234567890n"), "123456789012345678901234567890")
		require.Equal(t, ParsePseudoBigInt("0b1100011101110100100001111111101101100001101110011111000001110111001001110001111110000101011010010n"), "123456789012345678901234567890")
		require.Equal(t, ParsePseudoBigInt("0o143564417755415637016711617605322n"), "123456789012345678901234567890")
		require.Equal(t, ParsePseudoBigInt("0x18ee90ff6c373e0ee4e3f0ad2n"), "123456789012345678901234567890")
	})
}
