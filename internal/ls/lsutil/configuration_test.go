	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			case expectedPreferenceDefault:
				...
				Equal(t, defaultPref.QuotePreference, got.QuotePreference)

			case expectedPreferenceTS:
				assert.Equal(t, tt.config.ts.QuotePreference, got.QuotePreference)

			case expectedPreferenceJS:
				assert.Equal(t, tt.config.js.QuotePreference, got.QuotePreference)
...
func TestUserConfig_GetPreferences_CodeLensAndInlayHints(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			got := tt.config.GetPreferences(tt.activeFile)
			assert.Equal(t, tt.expectedLensesAndHints, got.CodeLens.ReferencesCodeLensEnabled)
			assert.Equal(t, tt.expectedLensesAndHints, got.InlayHints.IncludeInlayVariableTypeHints)
