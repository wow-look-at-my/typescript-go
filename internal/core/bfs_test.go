...
func TestBreadthFirstSearchParallel(t *testing.T) {
	...
	t.Run("basic functionality", func(t *testing.T) {
		...
		t.Run("find specific node", func(t *testing.T) {
			...
			require.Equal(t, result.Stopped, true, "Expected search to stop at D")
			require.Equal(t, result.Path, []string{"D", "B", "A"})
		})
		...
		t.Run("visit all nodes", func(t *testing.T) {
			...
			require.Equal(t, result.Stopped, false, "Expected search to not stop early")
			require.True(t, result.Path == nil, "Expected nil path when visit function never returns true")

			// Should visit all nodes exactly once
			sort.Strings(visitedNodes)
			expected := []string{"A", "B", "C", "D"}
			require.Equal(t, visitedNodes, expected)
		})
	})
	...
	t.Run("early termination", func(t *testing.T) {
		...
		require.True(t, visited.Has("Root"), "Expected to visit Root")
		require.True(t, visited.Has("L1A"), "Expected to visit L1A")
		require.True(t, visited.Has("L1B"), "Expected to visit L1B")
		require.True(t, visited.Has("L2A"), "Expected to visit L2A")
		require.True(t, visited.Has("L2B"), "Expected to visit L2B")
		// L2C is non-deterministic
		require.True(t, !visited.Has("L3A"), "Expected not to visit L3A")
	})
	...
	t.Run("returns fallback when no other result found", func(t *testing.T) {
		...
		require.Equal(t, result.Stopped, false, "Expected search to not stop early")
		require.Equal(t, result.Path, []string{"A"})
		require.True(t, visited.Has("B"), "Expected to visit B")
		require.True(t, visited.Has("C"), "Expected to visit C")
		require.True(t, visited.Has("D"), "Expected to visit D")
	})
