...
func TestIsReparsePoint(t *testing.T) {
	...
	t.Run("regular file", func(t *testing.T) {
		...
		require.Equal(t, isReparsePoint(file), false)
	})
	...
	t.Run("regular directory", func(t *testing.T) {
		...
		require.Equal(t, isReparsePoint(dir), false)
	})
	...
	t.Run("junction point", func(t *testing.T) {
		...
		require.Equal(t, isReparsePoint(link), true)
	})
	...
	t.Run("file symlink", func(t *testing.T) {
		...
		require.NoError(t, os.WriteFile(target, []byte("hello"), 0o666))
	...
	t.Run("nonexistent path", func(t *testing.T) {
		...
		require.Equal(t, isReparsePoint(nonexistent), false)
	})

	t.Run("empty path", func(t *testing.T) {
		t.Parallel()
		require.Equal(t, isReparsePoint(""), false)
	})

	t.Run("invalid path with null byte", func(t *testing.T) {
		t.Parallel()
		require.Equal(t, isReparsePoint("invalid\x00path"), false)
	})
}
...
func TestIsReparsePointLongPath(t *testing.T) {
	...
	require.NoError(t, os.MkdirAll(longTarget, 0o777))
	require.NoError(t, exec.Command("cmd", "/c", "mklink", "/J", longLink, longTarget).Run())
...
func TestIsReparsePointNestedInSymlink(t *testing.T) {
	...
	require.NoError(t, os.MkdirAll(innerTarget, 0o777))
	...
	require.Equal(t, isReparsePoint(nestedPath), true)
}
...
func TestIsReparsePointRelativePath(t *testing.T) {	//nolint:paralleltest // Cannot use t.Parallel() with t.Chdir()
	...
	require.Equal(t, isReparsePoint(link), true)
	require.Equal(t, isReparsePoint(target), false)
}
...
func BenchmarkIsSymlinkOrJunction(b *testing.B) {
	...
	require.NoError(b, os.WriteFile(regularFile, []byte("hello"), 0o666))

	target := filepath.Join(tmp, "target")
	link := filepath.Join(tmp, "link")
	require.NoError(b, os.MkdirAll(target, 0o777))
	require.NoError(b, exec.Command("cmd", "/c", "mklink", "/J", link, target).Run())
