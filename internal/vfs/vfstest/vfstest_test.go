...
func TestInsensitive(t *testing.T) {
	...
	require.Equal(t, sensitive, contents)
	sensitiveInfo, err := fs.Stat(vfs, "foo/bar/baz")
	require.NoError(t, err)
	require.Equal(t, sensitiveInfo.Sys(), 1234)
	sensitiveRealPath, err := vfs.Realpath("foo/bar/baz")
	require.NoError(t, err)
	require.Equal(t, sensitiveRealPath, "foo/bar/baz")
	entries, err := fs.ReadDir(vfs, "foo")
	require.NoError(t, err)
	require.Equal(t, dirEntriesToNames(entries), []string{"bar", "bar2", "bar3"})
	...
	require.NoError(t, fstest.TestFS(vfs, "foo/bar/baz"))

	insensitive, err := fs.ReadFile(vfs, "Foo/Bar/Baz")
	require.NoError(t, err)
	require.Equal(t, insensitive, contents)
	insensitiveInfo, err := fs.Stat(vfs, "Foo/Bar/Baz")
	require.NoError(t, err)
	require.Equal(t, insensitiveInfo.Sys(), 1234)
	insensitiveRealPath, err := vfs.Realpath("Foo/Bar/Baz")
	require.NoError(t, err)
	require.Equal(t, insensitiveRealPath, "foo/bar/baz")
	entries, err = fs.ReadDir(vfs, "Foo")
	require.NoError(t, err)
	require.Equal(t, dirEntriesToNames(entries), []string{"bar", "bar2", "bar3"})
...
func TestWritableFS(t *testing.T) {
	...
	content, ok = fs.ReadFile("/foo/bar/baz")
	require.True(t, ok)
	require.Equal(t, content, "goodbye, world")

	err = fs.WriteFile("/foo/bar/baz/oops", "goodbye, world", false)
	require.ErrorContains(t, err, `mkdir "foo/bar/baz": path exists but is not a directory`)
}
...
func TestWritableFSDelete(t *testing.T) {
	...
	require.True(t, fs.FileExists("/foo/bar/file.ts"))
	err := fs.Remove("/foo/bar/file.ts")
	require.NoError(t, err)
	require.True(t, !fs.FileExists("/foo/bar/file.ts"))

	_ = fs.WriteFile("/foo/bar/test/remove2.ts", "remove2", false)
	require.True(t, fs.DirectoryExists("/foo/bar/test"))
	err = fs.Remove("/foo/bar/test")
	require.NoError(t, err)
	require.True(t, !fs.FileExists("/foo/bar/test/remove2.ts"))
	require.True(t, !fs.DirectoryExists("/foo/bar/test"))
	...
	_ = fs.Remove("/foo/bar")
	require.True(t, fs.FileExists("/foo/barbar"))
}
...
func TestVFSTestMapFS(t *testing.T) {
	...
	t.Run("Realpath", func(t *testing.T) {
		...
		require.Equal(t, realpath, "/does/not/exist.ts")
	})
...
func TestVFSTestMapFSWindows(t *testing.T) {
	...
	t.Run("Realpath", func(t *testing.T) {
		...
		require.Equal(t, realpath, "c:/foo.ts")

		realpath = fs.Realpath("c:/Foo.ts")
		require.Equal(t, realpath, "c:/foo.ts")

		realpath = fs.Realpath("c:/does/not/exist.ts")
		require.Equal(t, realpath, "c:/does/not/exist.ts")
	})
}
...
func TestBOM(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			content, ok := fs.ReadFile("/foo.ts")
			require.True(t, ok)
			require.Equal(t, content, expected)
		})
	}
...
func TestSymlink(t *testing.T) {
	...
	t.Run("ReadFile", func(t *testing.T) {
		...
		require.True(t, ok)
		require.Equal(t, content, "this is existing.ts")
	})
	...
	t.Run("Realpath", func(t *testing.T) {
		...
		require.Equal(t, realpath, "/some/dir")

		realpath = fs.Realpath("/some/dirlink/file.ts")
		require.Equal(t, realpath, "/some/dir/file.ts")
	})

	t.Run("FileExists", func(t *testing.T) {
		t.Parallel()

		require.True(t, fs.FileExists("/symlink.ts"))
		require.True(t, fs.FileExists("/some/dirlink/file.ts"))
		require.True(t, fs.FileExists("/a/existing.ts"))
	})

	t.Run("DirectoryExists", func(t *testing.T) {
		t.Parallel()

		require.True(t, fs.DirectoryExists("/some/dirlink"))
		require.True(t, fs.DirectoryExists("/d"))
		require.True(t, fs.DirectoryExists("/c"))
		require.True(t, fs.DirectoryExists("/b"))
...
func TestWritableFSSymlink(t *testing.T) {
	...
	require.EqualError(t, err, `write "some/dirlink": path exists but is not a regular file`)

	// Can't write inside a broken dir symlink
	err = fs.WriteFile("/brokenlink/file.ts", "hello, world", false)
	require.EqualError(t, err, `broken symlink "brokenlink" -> "does/not/exist"`)
...
func TestWritableFSSymlinkChain(t *testing.T) {
	...
	require.True(t, ok)
	require.Equal(t, content, "this is new.ts")
...
func TestWritableFSSymlinkChainNotDir(t *testing.T) {
	...
	err := fs.WriteFile("/a/foo/bar/new.ts", "this is new.ts", false)
	require.EqualError(t, err, `mkdir "d": path exists but is not a directory`)
}
...
func TestWritableFSSymlinkDelete(t *testing.T) {
	...
	require.True(t, fs.DirectoryExists("/b"))
	require.True(t, fs.DirectoryExists("/c"))
	require.True(t, fs.FileExists("/d/existing.ts"))
	...
	require.True(t, !fs.DirectoryExists("/c"))
	require.True(t, !fs.DirectoryExists("/d"))
	require.True(t, !fs.FileExists("/d/again.ts"))
	err = fs.WriteFile("/d/again.ts", "d exists again", false)
	require.NoError(t, err)
	require.True(t, fs.DirectoryExists("/b"))
	require.True(t, fs.DirectoryExists("/c"))
	content, _ := fs.ReadFile("/b/again.ts")
	require.Equal(t, content, "d exists again")

	require.True(t, !fs.FileExists("/brokenlink"))
	require.True(t, !fs.DirectoryExists("/brokenlink"))
